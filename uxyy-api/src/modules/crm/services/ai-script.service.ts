...
export class AiScriptService {
  ...
  private readonly scriptTemplates: ScriptTemplate[] = [
    {
      ...
      category: '首次联系',
      title: '初次破冰',
      content: '您好，我是{company}的{name}，很高兴认识您。我了解到贵公司在{industry}领域很有建树，我们有一些相关的解决方案，不知道您是否有兴趣了解一下？',
      tags: ['初次', '礼貌', '专业'],
      effectiveness: 85,
    },
    {
      ...
      category: '需求挖掘',
      title: '痛点探询',
      content: '张总，目前贵公司在{area}方面，主要遇到哪些挑战呢？我们服务过很多同行业客户，他们普遍反馈{common_pain_point}，不知道您这边情况如何？',
      tags: ['提问', '倾听', '共情'],
      effectiveness: 90,
    },
    {
      ...
      category: '产品介绍',
      title: '价值呈现',
      content: '我们的产品可以帮助您解决{problem}，具体体现在：1）{benefit1}；2）{benefit2}；3）{benefit3}。像{case_company}这样的客户使用后，{case_result}。',
      tags: ['价值', '案例', '数据'],
      effectiveness: 88,
    },
    {
      ...
      category: '价格谈判',
      title: '价值锚定',
      content: '我理解您对价格的考虑。其实我们的产品虽然比市面上同类产品略高，但考虑到{value_point}，以及{roi_calculation}，实际上您的投入产出比会更高。我们可以根据您的采购量，申请一个优惠方案。',
      tags: ['价值', '对比', '让步'],
      effectiveness: 82,
    },
    {
      ...
      category: '异议处理',
      title: '顾虑化解',
      content: '您提到的{objection}确实是很多客户关心的问题。实际上，{explanation}。另外，我们还提供{guarantee}，确保您没有后顾之忧。',
      tags: ['认同', '解释', '保障'],
      effectiveness: 87,
    },
    {
      ...
      category: '促成成交',
      title: '限时促单',
      content: '张总，如果您今天能确定下来，我可以帮您申请{special_offer}。这个优惠名额有限，而且{urgency_reason}。您看是签{option1}还是{option2}更合适？',
      tags: ['紧迫', '选择', '优惠'],
      effectiveness: 80,
    },
    {
      ...
      category: '售后跟进',
      title: '满意度回访',
      content: '张总，产品使用了一段时间了，您感觉怎么样？有没有遇到什么问题？我们非常重视您的反馈，有任何需要随时联系我。',
      tags: ['关怀', '服务', '维护'],
      effectiveness: 92,
    },
    {
      ...
      category: '挽回客户',
      title: '流失挽回',
      content: '张总，好久不见了。最近我们推出了{new_feature}，我觉得特别适合贵公司。不知道您最近有没有这方面的需求？我们可以约个时间详细聊聊。',
      tags: ['新品', '唤醒', '邀约'],
      effectiveness: 75,
    },
  ];
  ...
  async generateScript(
    ...
    for (const template of templates) {
      const script = await this.fillTemplate(template, customer, followUps, opportunities);
      scripts.push(script);
    }
  ...
  async recommendScriptByCustomerStatus(
    ...
    const daysSinceLastFollowUp = lastFollowUp[0]
      ? Math.floor((Date.now() - new Date(lastFollowUp[0].createdAt).getTime()) / (1000 * 60 * 60 * 24))
      : null;

    // 根据客户状态推荐话术
    if (followUpCount[0]?.count === 0) {
      // 新客户
      const script = await this.generateScript(customerId, '首次联系', enterpriseId);
      scripts.push(...script);
    } else if (orderCount[0]?.count > 0 && daysSinceLastFollowUp && daysSinceLastFollowUp > 30) {
      // 老客户，长时间未联系
      const script = await this.generateScript(customerId, '售后跟进', enterpriseId);
      scripts.push(...script);
    } else if (daysSinceLastFollowUp && daysSinceLastFollowUp > 14) {
      // 跟进中断
      const script = await this.generateScript(customerId, '挽回客户', enterpriseId);
      scripts.push(...script);
    } else {
      // 常规跟进
      const script = await this.generateScript(customerId, '需求挖掘', enterpriseId);
      scripts.push(...script);
    }
