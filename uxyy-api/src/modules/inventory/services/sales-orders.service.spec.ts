...
function makeMockDb(): { db: MockDb; tx: MockDb } {
  // tx is thenable so await tx.update().set().where() resolves
  ...
  // db must NOT be thenable — NestJS DI would resolve it as a Promise
  const db: MockDb = {
    select: jest.fn().mockReturnThis(),
    from: jest.fn().mockReturnThis(),
    where: jest.fn().mockReturnThis(),
...
describe('SalesOrdersService', () => {
  ...
  describe('state machine transitions', () => {
    it('should reject outbound when order is draft', async () => {
      tx.execute.mockResolvedValue({ rows: [{ id: 1, status: 'draft' }] });

      await expect(service.outbound(1, 1, { items: [] }, 1)).rejects.toThrow(
        BadRequestException,
      );
    });

    it('should reject outbound when order is completed', async () => {
    ...
    it('should reject outbound when order is cancelled', async () => {
      tx.execute.mockResolvedValue({ rows: [{ id: 1, status: 'cancelled' }] });
    ...
    it('should allow cancel on draft order', async () => {
      tx.execute.mockResolvedValue({ rows: [{ id: 1, status: 'draft' }] });

      await expect(service.cancel(1, 1, 1)).resolves.toEqual({
        ok: true,
        orderId: 1,
        stockRestored: false,
      });
    ...
    it('should reject cancel on completed order', async () => {
      tx.execute.mockResolvedValue({ rows: [{ id: 1, status: 'completed' }] });

      await expect(service.cancel(1, 1, 1)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
