...
export class OcrProcessor extends WorkerHost {
  ...
  async process(job: Job<{ imageUrl: string; enterpriseId: number }>): Promise<OcrResult> {
    this.logger.log(`Processing OCR job ${job.id} for image: ${job.data.imageUrl}`);
...
export class SuggestionProcessor extends WorkerHost {
  ...
  async process(job: Job<{ type: string; enterpriseId: number }>): Promise<any> {
    this.logger.log(`Processing suggestion job ${job.id} for type: ${job.data.type}`);
