...
1. `多智能体并行开发指南.md`
2. PRD（财务章节、**§2.5.5 MVP 财务报表口径**、单据→凭证链路、**11.5.2**）；**发票新建页 OCR 填入** 见 `docs/PRD-补充-发票新建-OCR填入.md`
3. `docs/各智能体职责与任务说明.md` → **Agent-Finance**

**实现提示（与本仓库代码对齐）**：

- 凭证列表为 **单行分录**模型；列表筛选含 `sourceType`（如 `ai_task`），**勿**在查询层使用草稿/过账 `status`。  
- 资产负债表 / 利润表 / 现金流量简版：按 `finance.service.ts` 中 **凭证 + 科目表** 聚合逻辑与 PRD §2.5.5 验收。  
- 应收应付：来自 **已验证未入账发票** 的拆分规则见 PRD §2.5.5。
