...
export class FinanceService {
  ...
  async findVouchersPage(params: {
    ...
    if (params.startDate) {
      whereClause = and(whereClause, gte(schema.vouchers.date, new Date(params.startDate)))!;
    }
    if (params.endDate) {
      whereClause = and(whereClause, lte(schema.vouchers.date, new Date(params.endDate)))!;
    }
    ...
    const rows = await this.db
      ...
      .from(schema.vouchers)
      .where(whereClause)
      .orderBy(desc(schema.vouchers.date))
      .limit(params.pageSize)
      .offset(offset);
  ...
  async createVoucher(enterpriseId: number | undefined, dto: CreateVoucherDto) {
    ...
    const totalAmount = dto.entries.reduce((sum, e) => sum + (e.debit || 0), 0);
