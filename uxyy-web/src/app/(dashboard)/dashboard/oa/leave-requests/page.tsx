...
export default function LeaveRequestsPage() {
  ...
  const filteredRequests = useMemo(() => {
    return rows.filter((req) => {
      ...
      if (searchQuery && !String(req.reason ?? "").includes(searchQuery))
        return false;
      return true;
    });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-text-tertiary mt-1">
            事假、病假、年假申请；本列表按企业展示<strong>全员</strong>
            申请记录。 具有审批权限的成员请在详情中使用<strong>审批操作</strong>
            ，或通过 OA 首页「待审批中心」处理。
          </p>
        </div>
        <Button asChild>
          ...
          {q.isLoading ? (
            ...
            <div className="space-y-4">
              {filteredRequests.map((req) => {
                ...
                const tone =
                  typeTone[req.type] ?? "bg-bg-tertiary text-text-secondary";
                return (
                  <div
                    key={req.id}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-text-primary mt-1">
                          {formatDay(req.startDate)} 至 {formatDay(req.endDate)}{" "}
                          · {req.days}天
                        </p>
