...
function ForbiddenBody() {
  ...
  const needList = parseNeedParam(need ?? null);
  const canon = useAuthStore((s) => s.canonicalRole);

  return (
    <div className="flex max-w-lg flex-col gap-4">
      <h1 className="text-lg font-semibold text-text-primary">当前账号无权访问该页面</h1>
      <Card className="p-6 space-y-3">
        ...
        {needList.length > 0 ? (
          <div className="rounded-md border border-border-primary bg-bg-secondary px-3 py-2 text-xs text-text-secondary space-y-1.5">
            <p className="font-medium text-text-primary">缺少以下权限（任一即可时，以页面规则为准）</p>
            <ul className="list-disc pl-4 space-y-1">
              {needList.map((code) => (
                <li key={code}>
                  <span className="text-text-secondary">{hintForPermissionCode(code)}</span>
                  <code className="ml-1 text-text-tertiary">{code}</code>
                </li>
              ))}
            </ul>
          </div>
