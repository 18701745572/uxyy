...
* **批量导出与导入（已实现，列对齐）**：
  - **导出**：客户列表页支持导出 **Excel（.xlsx）** 与 **CSV**，表头为中文：`客户名称`、`联系人`、`电话`、`地址`、`类型`、`等级`、`行业`、`创建时间`（与 **`GET /api/v1/export/customers`** 一致）。
  - **导入**：**`POST /api/v1/crm/customers/import`**（`multipart/form-data`，字段名 **`file`**），支持 **xlsx / xls / csv**，单文件 ≤ **5MB**。表头与导出一致即可导入；`创建时间` 列忽略；可选列 **`备注`**、**`信用额度`**（或 **`授信额度`**）可一并导入。查询 **`mode`**：`skip`（默认，同企业下名称+电话已存在则跳过）| `force`（按创建接口 `force` 规则处理重复，仍受校验约束）。
  - **前端**：路由 **`/dashboard/crm/customers`** 提供「批量导入」，展示成功数、跳过数及失败行号与原因。
  - **权限**：导入需 **`crm:write`**（与创建客户一致）。
...
| 7 | 实现客户导入API（xlsx/xls/csv，与导出列对齐；`POST /api/v1/crm/customers/import`） | P1 | 可批量导入；`mode=skip|force` | 任务2 |
