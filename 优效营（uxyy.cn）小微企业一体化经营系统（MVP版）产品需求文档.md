...
| **服务支持** | 社区文档+AI客服 | 在线客服+视频教程+社群 | 1v1客户成功经理+专属支持 |
...
#### 2.5.5 MVP 财务报表口径说明（资产负债表 / 利润表 / 应收应付 / 现金流量简表）
...
| **V2（Phase 3）** | 商机成单预测 | ⭐⭐⭐☆☆ 中 | 帮助销售聚焦重点 | 需积累足够数据后，使用轻量机器学习模型 |
| | 客户流失预警 | ⭐⭐⭐☆☆ 中 | 提醒及时跟进 | 基于「未跟进天数+未成交天数」的规则+轻量模型 |
| **V3（Phase 4）** | 智能话术推荐 | ⭐⭐☆☆☆ 低 | 提升销售转化率 | 需大模型+行业知识库，成本较高 |
| | 智能采购预测 | ⭐⭐☆☆☆ 低 | 优化库存资金占用 | 需多维度数据（季节、促销、供应链周期） |
...
* 商机成单预测：基于客户跟进频率、历史成交数据，预测成单概率（准确率≥70%），预测结果同步至 [uxyy.cn](https://uxyy.cn) CRM 模块首页；

* 客户流失预警：识别长期未跟进、无交易记录的客户，推送挽回建议至 [uxyy.cn](https://uxyy.cn) 消息中心；

* 智能话术推荐：基于客户需求标签（如 “价格敏感”“关注质量”），推荐对应沟通话术，话术同步至 [uxyy.cn](https://uxyy.cn) CRM 跟进模块。
