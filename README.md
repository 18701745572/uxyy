...
- **数据库**：PostgreSQL（生产默认 [**Neon**](https://neon.tech/) Serverless Postgres）+ **Drizzle ORM** + **drizzle-kit**。**本地开箱**：仓库根 **`docker compose up -d postgres`**，连接串见 [`uxyy-api/.env.example`](./uxyy-api/.env.example)；迁移在 `uxyy-api` 目录执行 **`pnpm run db:migrate`**。
